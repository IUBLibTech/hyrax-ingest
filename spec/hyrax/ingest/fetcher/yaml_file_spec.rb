...
RSpec.describe Hyrax::Ingest::Fetcher::YAMLFile do
  let(:sip) { Hyrax::Ingest::SIP.new(path: "#{fixture_path}/sip_examples/ingest_work_from_yaml") }
  ...
  describe '.new' do
    context 'when missing :filename keyword argument' do
      it 'raises an ArgumentError' do
        expect { described_class.new(sip, yaml_path: 'foo') }.to raise_error ArgumentError
      end
    end
